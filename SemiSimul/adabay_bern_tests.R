#' @title Regression tests for adabay_bern.R
#' @description Pin down the mathematical correctness of the kernel against
#'   hand-computed references. Run after any change to `adabay_bern.R`.
...
kernel_candidates <- c(
  "./Code/Code v1.0/adabay_bern.R",
  "./adabay_bern.R"
)
kernel_path <- kernel_candidates[file.exists(kernel_candidates)][1L]
if (is.na(kernel_path)) {
  stop("Cannot locate adabay_bern.R from PROJECT_ROOT = ", PROJECT_ROOT)
}
source(kernel_path)
...
# ---- Test 6: within-tolerance selection rule in initialisePriorSettings -------------
# §2.3 within-tolerance rule: pick the smallest L at which the running-minimum empirical
...
cat("\n=== Test 6: within-tolerance selection rule ===\n")
{
  set.seed(21L)
  ...
  # fit for both the within-tolerance and the max rules, which doesn't exercise the
  # selection logic.
  samples <- 1 / (1 + exp(-rnorm(50000L, mu, sigma)))
  ps_wtol <- initialisePriorSettings(priorSamples       = list(samples),
  ...
  ps_aic   <- initialisePriorSettings(priorSamples       = list(samples),
                                      numberOfComponents = 4L,
                                      selection          = "aic")
  L_wtol <- length(ps_wtol[[1]]$weight)
  L_max   <- length(ps_max[[1]]$weight)
  L_aic   <- length(ps_aic[[1]]$weight)
  cat(sprintf("  within-tolerance L = %d; aic L = %d; max L = %d\n",
              L_wtol, L_aic, L_max))
  stopifnot(L_wtol >= 1L, L_wtol <= 4L)
  stopifnot(L_max   >= 1L, L_max   <= 4L)
  stopifnot(L_wtol <= L_max)         # within-tolerance is no larger than max
  # All three selections produce valid mixtures (weights sum to 1; positive
  # alpha/beta).
  for (ps in list(ps_wtol, ps_aic, ps_max)) {
    stopifnot(abs(sum(ps[[1]]$weight) - 1) < 1e-8)
    stopifnot(all(ps[[1]]$alpha > 0), all(ps[[1]]$beta > 0))
  }
  cat("  PASS: all three selection rules return valid mixtures with L_wtol <= L_max\n")
}

...
// (see the .init_bayseqSim() R-side toggle below) and ensure your toolchain
...
// [[Rcpp::export]]
NumericMatrix bayseqSim_tail_prob(
    NumericVector nodes,
...
// [[Rcpp::export]]
NumericMatrix bayseqSim_density(
    NumericVector nodes,
    NumericVector weights,
    IntegerVector ec,
    IntegerVector et,
  ...
  // likelihood; see comment on `bayseqSim_tail_prob` for details.
  std::vector<double> lB_prior_c((std::size_t)Lc);
...
.init_bayseqSim <- function() {
  ...
  .bayseqSim_env$gl_nodes   <- (1 + nodes_pm1) / 2
  .bayseqSim_env$gl_weights <- wts / 2
  ...
  .bayseqSim_env$use_rcpp <- FALSE
  if (requireNamespace("Rcpp", quietly = TRUE)) {
    tryCatch({
      env_args <- list(code     = .bayseqSim_env$cpp_src,
                       env      = .bayseqSim_env,
                       verbose  = FALSE,
                       cacheDir = tempdir(),
                       rebuild  = FALSE)
      ...
      .bayseqSim_env$use_rcpp <- TRUE
    }, error = function(e) {
      message("bayseqSim: Rcpp kernel unavailable; using pure-R quadrature. (",
              conditionMessage(e), ")")
      .bayseqSim_env$use_rcpp <- FALSE
    })
  }
...
# `bayseqSim_tail_prob` and is used only when the Rcpp kernel is unavailable.
...
.posterior_tail_prob <- function(ec, et, mc, nt, priors_c, priors_t,
  ...
  nodes   <- .bayseqSim_env$gl_nodes
  weights <- .bayseqSim_env$gl_weights
  ...
  fn <- if (isTRUE(.bayseqSim_env$use_rcpp)) {
    .bayseqSim_env$bayseqSim_tail_prob
  } else {
    .tail_prob_R
  }
...
.posterior_density <- function(ec, et, mc, nt, priors_c, priors_t,
  ...
  fn <- if (isTRUE(.bayseqSim_env$use_rcpp)) {
    .bayseqSim_env$bayseqSim_density
  } else {
    .density_R
  }
...
#'   * "elbow" (default; matches Section 2.3 of the manuscript): the smallest
#'     L whose empirical forward KL divergence is within `elbow_eps` of the
...
#'     recommended.
#' @param elbow_eps tolerance for the "elbow" selection rule. Default 1e-3
#'   as adopted throughout the paper.
initialisePriorSettings <- function(priorSamples = NULL,
                                    numberOfComponents = 4L,
                                    selection = c("elbow", "aic", "max"),
                                    elbow_eps = 1e-3) {
  ...
  for (i in seq_along(priorSamples)) {
    if (all(is.na(priorSamples[[i]]))) {
      ...
      bm <- switch(selection,
                   ...
                   "elbow" = .select_elbow(fits, priorSamples[[i]], elbow_eps))
      priors[[i]] <- list(
...
.select_elbow <- function(fits, samples, epsilon) {
...
runTrialMonitoring <- function(simulationSettings,
  ...
  message("calculating posterior probabilities at stage 1 ... ")
  ec_mat[, 1L] <- rbinom(
    n    = R,
